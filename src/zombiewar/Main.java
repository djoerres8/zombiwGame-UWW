...
public class Main {
  ...
  public static IZombie[] randomZombies() {
    ...
    for (int i = 0; i < zombies.length; i++) {
      ...
      switch(zombieType){
        case 0: zombies[i] = (IZombie) factory.make("common infect"); break;
  ...
  public static void main(String[] args) {
    ...
    System.out.println("We have " + survivors.length + " survivors trying to make it to safety (" + child + " children, " + teacher + " teachers, " + soldier + " soldiers)");
    
    int common = 0;
    int tank = 0;
    for(int i = 0; i < zombies.length; i++){
    	if(zombies[i] instanceof Tank) tank++;
    	if(zombies[i] instanceof CommonInfected) common++;
    }
    
    System.out.println("But there are " + zombies.length + " zombies waiting for them (" + common + " common infected, " + tank + " tanks)");
